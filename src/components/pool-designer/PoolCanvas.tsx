...
export const PoolCanvas: React.FC<PoolCanvasProps> = ({ imageFile, scaleInfo, className, canvasOnly = false, onStateChange }) => {
  ...
  const addGoogleScaleReference = async (canvas: FabricCanvas, scaleInfo: { metersPerPixel: number }, bgImage: FabricImage) => {
    ...
    // Group scale elements (no background)
    const scaleGroup = new Group([scaleBar, leftTick, rightTick, scaleText], {
      selectable: false,
      evented: false,
    });
  ...
  // Function to add an independent, rotatable north indicator for regular property images
  const addPropertyNorthIndicator = async (canvas: FabricCanvas) => {
    const northSize = 60;
    ...
    (northGroup as any).isPropertyNorthIndicator = true;
    ...
    reader.onload = (e) => {
      ...
      FabricImage.fromURL(imgSrc).then((img) => {
        ...
        if (scaleInfo) {
          ...
          // Add independent rotatable north indicator for regular property images
          addPropertyNorthIndicator(fabricCanvas);
        }
  ...
  const addRectangularPaver = (widthFeet: number, lengthFeet: number) => {
    ...
    paver.controls['deleteControl'] = new Control({
      ...
      cursorStyle: 'pointer',
      mouseUpHandler: () => {
        // Remove paver only (no area text to remove)
        fabricCanvas.remove(paver);
        setPavers(prev => prev.filter(p => p !== paver));
        fabricCanvas.renderAll();
        return true;
