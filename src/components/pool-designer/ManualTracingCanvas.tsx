...
import waterTextureImg from '@/assets/water-texture.jpg';
...
// Custom pool dimensions input
  ...
  const completeShape = useCallback(() => {
    ...
    if (mode === 'property') {
      ...
      // Calculate per-side paver offsets in pixels
    ...
    // Add vertex markers for editing (skip for pools and pavers - no corner circles)
    const newMarkers: Circle[] = [];
    if (mode !== 'pool' && mode !== 'paver') {
      const markerColor = mode === 'property' ? '#22c55e' : '#000000';
  ...
  const calculatePolygonAreaSqFt = (points: { x: number; y: number }[]): number => {
    ...
    // Convert from pixels² to meters² then to feet² using dynamic scale
    const currentScale = scalePixelsPerMeterRef.current;
    const areaMeters = area / (currentScale * currentScale);
  ...
  const addMeasurementFromInput = () => {
    ...
    // Use dynamic scale instead of static constant
    const lengthPixels = lengthInMeters * getPixelsPerMeter();
  ...
  const addRectangularPaver = () => {
    ...
    // Create rectangle with solid black 0.5px stroke
    const rect = new Rect({
      left: centerX,
      top: centerY,
      width: widthPixels,
